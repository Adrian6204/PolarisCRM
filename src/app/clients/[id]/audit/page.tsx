...
function DiffView({ diff }: { diff: unknown }) {
  ...
  if (d.changed) {
    ...
    if (entries.length === 0) return <span className="text-muted">no field changes</span>;
    return (
      <ul className="flex flex-col gap-0.5">
        {entries.map(([field, { from, to }]) => (
          <li key={field} className="text-xs">
            <span className="font-medium">{field}</span>:{" "}
            <span className="text-muted line-through">{fmt(from)}</span>{" "}
...
export default async function ClientAuditPage({
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {trail.length === 0 ? (
        <p className="rounded border border-dashed border-line-strong p-8 text-center text-sm text-muted">
